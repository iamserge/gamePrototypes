	...
	return function collectionOptionsViewModel(config) {
		var mapOptions = {
			...
			sendRequest = function(searchQuery){
				...
				$.get(config.RequestUrl + '?postalCode=' + searchQuery , function(res) {
			...
			cacheCollectionDetailsContainer = function(){
				...
				if (typeof $collectionPointDetails != "undefined") {
					...
					$(config.CollectionPointsContainerSelector).after($collectionPointDetails)
				}

			}
			scrollToCollectionOptions = function(){
				var scrollTo = (typeof self.UserLocation().lat == 'undefined')? $(config.CollectionOptionsSelector).offset().top - 50 : $(config.UserLocationMapSelector).offset().top - 245;
				$("html, body").animate({ scrollTop: scrollTo});
			},

			setupCollectionPointDetails = function(model, $element){
				$collectionPointDetails = $(config.CollectionPointDetailsSelector);
				...
				setupGoogleMap(self.SelectedCollectionPoint().latitude, self.SelectedCollectionPoint().longitude, $collectionPointDetails.find(config.CollectionPointDetailsMapSelector)[0]);
			},
			...
			geoCodeCoordinates = function(latitude, longitude) {
				$.get(config.GeoCodingURL + config.GeoCodingKey + '&latlng=' + latitude + ',' + longitude, function(res){
			...
			setupUserLocationMap = function(closestPostcodeObject){
				...
				var $userLocationContainer = $(config.UserLocationContainerSelector);
				self.UserLocation(closestPostcodeObject.geometry.location);
				sendRequest(self.Location());
				setupGoogleMap(self.UserLocation().lat, self.UserLocation().lng, $userLocationContainer.find(config.UserLocationMapSelector)[0])
			};
